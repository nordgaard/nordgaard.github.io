...
if ( ! function_exists( 'helena_post_page_category_slider' ) ) :
	...
	function helena_post_page_category_slider() {
		...
		elseif ( 'featured-category-slider' == $type ) {
			...
			$args['category__in'] = (array) apply_filters( 'helena_get_option', 'featured_slider_select_category' );
		}
		...
		while ( $get_featured_posts->have_posts()) : $get_featured_posts->the_post(); $i++;
			...
			$classes[] = 'post post-'.$post->ID.' hentry slides';
			...
			<article class="' . esc_attr( implode( ' ', $classes ) ) . '">
				...
				if ( has_post_thumbnail() ) {
					$output .= '<a title="' . $title_attribute . '" href="' . esc_url( get_permalink() ) . '">
						'. get_the_post_thumbnail( $post->ID, 'helena-slider', array( 'title' => $title_attribute, 'alt' => $title_attribute, 'class'	=> 'attached-post-image' ) ).'
					</a>';
				}
				else {
					...
					$first_image = helena_get_first_image( $post->ID, 'helena-slider', array( 'title' => $title_attribute, 'alt' => $title_attribute, 'class' => 'attached-post-image' ) );
