...
if ( ! function_exists( 'helena_post_page_category_logo_slider' ) ) :
	...
	function helena_post_page_category_logo_slider() {
		...
		elseif ( 'category' == $type ) {
			$no_of_post           = $quantity;
			$args['category__in'] = (array) apply_filters( 'helena_get_option', 'logo_slider_select_category' );
		}
		...
		while ( $get_hero_posts->have_posts() ) {
			...
			$output .= '
				...
				if ( has_post_thumbnail() ) {
					...
					$first_image = helena_get_first_image( $post->ID, 'full', array( 'title' => $title_attribute, 'alt' => $title_attribute ) );
